...
git_fzf() {
  ...
  show_help() {
    ...
    echo
    echo "Examples:"
    echo "  git_fzf -c             Checkout a branch"
    echo "  git_fzf -d             Delete a branch"
    echo "  git_fzf -r -m          Merge a remote branch into the current branch"
    echo "  git_fzf -p             Pull the latest changes after checking out a branch"
  }
  ...
  if [[ $flag == "-r" ]]; then
    ...
    if [[ -n "$selected_branch" ]]; then
      if [[ $action == "delete" ]]; then
        ...
        read -rp "Enter new branch name: " new_branch_name
    ...
    if [[ -n "$selected_branch" ]]; then
      ...
      if [[ $action == "delete" ]]; then
        ...
        if [[ $action == "merge" ]]; then
          ...
          if [[ "$(pwd)" == *alameda* ]]; then
            cd "$HOME/mcisemi/alameda" || exit
            local target_dir
            target_dir="$(fd -t d "$selected_branch")"
