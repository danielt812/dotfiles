...
function alameda_push() {
  ...
  if [[ $branch =~ ^(master|staging)$ ]]; then
    ...
    if [[ $flag == '-a' ]]; then
      git pull && git push && git checkout master && git pull && git merge $branch --no-edit && git push && git checkout staging && git pull && git merge $branch --no-edit && git push && git checkout $branch
    elif [[ $flag == '-m' ]]; then
      git pull && git push && git checkout master && git pull && git merge $branch --no-edit && git push && git checkout $branch
    elif [[ $flag == '-s' ]]; then
      git pull && git push && git checkout staging && git pull && git merge $branch --no-edit && git push && git checkout $branch
    elif [[ $flag == '-h' ]]; then
      echo 'Usage:'
